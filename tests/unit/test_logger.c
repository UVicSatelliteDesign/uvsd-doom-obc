...
{
    /* When the log is full, writing must return LOG_ERR_FULL and must
...
{
    /* LOG_CRITICAL entries must be preserved even when the log wraps. */
    TEST_IGNORE_MESSAGE("TODO: implement when logger.c exists");
}
...
{
    /* A message longer than LogEntry_t.message must be truncated, not
     * overrun the buffer. */
    TEST_IGNORE_MESSAGE("TODO: implement when logger.c exists");
}
...
{
    /* Every LogLevel_t value must be accepted as a valid level. */
    TEST_IGNORE_MESSAGE("TODO: implement when logger.c exists");
}
